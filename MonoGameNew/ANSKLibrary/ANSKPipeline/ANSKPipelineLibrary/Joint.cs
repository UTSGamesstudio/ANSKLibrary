...
namespace ANSKPipelineLibrary
{
    public class Joint
    {
        ...
        public void ExportToXML(XmlWriter writer)
        {
            ...
            writer.WriteAttributeString("Translation", XmlExporter.MatrixToEntity(_translation));
            writer.WriteAttributeString("Rotation", XmlExporter.MatrixToEntity(_rotation));
            writer.WriteAttributeString("Scale", XmlExporter.MatrixToEntity(_scale));
            writer.WriteEndElement();

            writer.WriteStartElement("Indices");
            for (int i = 0; i < _indices.Count; i++)
                writer.WriteElementString("Indice", _indices[i].ToString());
            writer.WriteEndElement();

            writer.WriteStartElement("Weights");
            for (int i = 0; i < _weights.Count; i++)
                writer.WriteElementString("Weight", _weights[i].ToString());
            writer.WriteEndElement();

            writer.WriteEndElement();

            for (int i = 0; i < _children.Count; i++)
                _children[i].ExportToXML(writer);
        }
    }
}
