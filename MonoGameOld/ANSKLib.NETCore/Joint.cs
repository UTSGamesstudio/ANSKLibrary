...
namespace ANSKLibrary
{
    public class Joint
    {
        ...
        public void ToXML(ref XmlWriter writer)
        {
            ...
            writer.WriteAttributeString("ParentId", _parentId.ToString());

            writer.WriteAttributeString("Translation", XmlHelper.MatrixToXML(_translation));
            writer.WriteAttributeString("Rotation", XmlHelper.MatrixToXML(_rotation));
            writer.WriteAttributeString("Scale", XmlHelper.MatrixToXML(_scale));

            writer.WriteStartElement("Children");
            writer.WriteAttributeString("Count", _children.Count.ToString());
            for (int i = 0; i < _children.Count; i++)
                writer.WriteElementString("ChildJoint", _children[i]._id.ToString());
            writer.WriteEndElement();

            writer.WriteAttributeString("Indices", XmlHelper.ListIntToXML(_indices));
            writer.WriteAttributeString("Weights", XmlHelper.ListFloatToXML(_weights));

            writer.WriteEndElement();

            // Now we have to continue through the tree and ANSKSerialise.
            for (int i = 0; i < _children.Count; i++)
                _children[i].ToXML(ref writer);
        }
    }
}
